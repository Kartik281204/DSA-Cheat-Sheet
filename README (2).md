...
1. **Go in order.** The patterns are sequenced roughly by increasing difficulty and by dependency (e.g. Recursion before Backtracking before Trees before Graphs).
2. **Read the idea, then close the tab.** Don't jump straight to problems with the template open — understand *why* it works first.
...
**⚠️ Common pitfall:** forgetting the "un-choose" step (`path.pop()`) — without it, choices from one branch leak into sibling branches. Before writing any backtracking code, sketch the recursion tree on paper; the base case and the pruning condition both become obvious once you can see the shape of the search.
